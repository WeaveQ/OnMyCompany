...
/**
 * Prefixes/paths for company product APIs (member session enforced in handlers).
 * Keep in sync with docs/onmycompany/API-NOTES.md public-prefix table.
 */
export const COMPANY_PRODUCT_PUBLIC_PREFIXES = [
  "/api/company/",
  "/api/me/",
  "/api/org/",
  "/api/catalog/",
...
/** Exported for tests / docs checks: ops-admin middleware bypass list. */
export function isCompanyProductPublicPath(path: string, method = "GET"): boolean {
...
/**
 * Whether this request may act as local ops-admin (console unlock).
...
export async function isLocalAdminAuthenticated(context: Context, adminToken: string | undefined): Promise<boolean> {
  ...
  return hasRequestToken(context, token);
}
...
// Deployment secrets (`OMC_ADMIN_TOKEN` / `OMC_RUNTIME_TOKEN`; deprecated aliases via env.ts) are long-lived,
