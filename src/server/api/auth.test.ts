...
import { createLocalAuthMiddleware, isCompanyProductPublicPath } from "./auth.ts";
...
describe("createLocalAuthMiddleware", () => {
  ...
  it("bypasses ops-admin for company product paths including /api/teams", async () => {
    const app = new Hono();
    app.use("*", createLocalAuthMiddleware({ adminToken: "admin-secret" }));
    app.get("/api/teams", (context) => context.json({ items: [] }));
    app.get("/api/teams/:teamId/members", (context) => context.json({ items: [] }));
    app.get("/api/company/health", (context) => context.json({ ok: true }));
    app.get("/api/catalog/skills", (context) => context.json({ items: [] }));
    app.get("/api/connections", (context) => context.json({ ok: true }));

    // Without ops-admin bearer: company product paths still reach handlers (200).
    expect((await app.request("/api/teams")).status).toBe(200);
    expect((await app.request("/api/teams/abc/members")).status).toBe(200);
    expect((await app.request("/api/company/health")).status).toBe(200);
    expect((await app.request("/api/catalog/skills")).status).toBe(200);
    // Non-public admin surface still locked.
    expect((await app.request("/api/connections")).status).toBe(401);

    expect(isCompanyProductPublicPath("/api/teams")).toBe(true);
    expect(isCompanyProductPublicPath("/api/teams/x/members")).toBe(true);
    expect(isCompanyProductPublicPath("/api/connections")).toBe(false);
  });

  it("exports company public prefixes covering all product route roots", async () => {
    const { COMPANY_PRODUCT_PUBLIC_PREFIXES } = await import("./auth.ts");
    expect(COMPANY_PRODUCT_PUBLIC_PREFIXES).toEqual(
      expect.arrayContaining(["/api/company/", "/api/me/", "/api/org/", "/api/catalog/", "/api/teams"]),
    );
    for (const sample of [
      ...
      expect(isCompanyProductPublicPath(sample)).toBe(true);
    }
  });
});
