...
describe("ConnectServer", () => {
  ...
  it("rejects deeply nested idempotent inputs before claiming the key", async () => {
    ...
    // ActionRunner resolves a connection slot before dispatch (default name).
    const connection = await app.request("/api/connections/example", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ authType: "api_key", values: { apiKey: "example-key" } }),
  ...
  it("filters run logs by action, caller, and status and returns run details", async () => {
    ...
    expect((await app.request(`/api/runs?memberId=${"m".repeat(129)}`)).status).toBe(400);
  });

  it("filters runs by memberId", async () => {
    const runs = new MemoryRunLogStore();
    await runs.add({
      ...createRunLog("run-a", "2026-06-30T00:00:02.000Z"),
      memberId: "member-aaa",
    });
    await runs.add({
      ...createRunLog("run-b", "2026-06-30T00:00:01.000Z"),
      memberId: "member-bbb",
    });
    const app = createTestServer([apiKeyProvider], { runs }).createApp();

    const response = await app.request("/api/runs?memberId=member-aaa");
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      items: [{ id: "run-a", memberId: "member-aaa" }],
    });
  });
});
  ...
  async list(input: RunLogListInput = {}): Promise<RunLogPage> {
    ...
    const filteredRuns = this.runs.filter(
      (run) =>
        ...
        (!input.memberId || run.memberId === input.memberId),
    );
    const start = cursor
