...
export interface IConnectServerOptions {
  ...
  /**
   * When true (OnMyCompany product mount), direct PUT /api/runtime-policy is
...
export class ConnectServer {
  ...
  createApp(): Hono {
    ...
    // G0: cap concurrent execution on hot paths (after auth so member id is available).
    app.use("/v1/actions/*", (context, next) => this.withConcurrencyCap(context, next));
    app.use("/v1/proxy/*", (context, next) => this.withConcurrencyCap(context, next));
    app.use("/mcp", (context, next) => this.withConcurrencyCap(context, next));
    app.get("/v1/health", (context) => writeRuntimeSuccess(context, { ok: true, runtime: "onmycompany" }));
    ...
    app.get(
      "/docs",
      Scalar({
        pageTitle: "OnMyCompany API Reference",
        url: "/openapi.json",
        theme: "default",
        darkMode: false,
  ...
  /** G0 concurrency cap for execution paths (POST actions / proxy / MCP). */
  private async withConcurrencyCap(context: Context, next: () => Promise<void>): Promise<Response | void> {
    if (context.req.method === "GET" || context.req.method === "HEAD" || context.req.method === "OPTIONS") {
      return next();
    }
    const guard = getSharedConcurrencyGuard();
    const grant = readRuntimeGrant(context);
    const memberKey = grant?.memberId || grant?.tokenId || context.req.header("x-forwarded-for") || "anonymous";
    const slot = guard.acquire(memberKey);
    if (!slot.ok) {
      ...
      return jsonError(
        ...
        "rate_limited",
        slot.reason === "member"
          ? "Too many concurrent requests for this member. Retry later."
          : "Gateway is at capacity. Retry later.",
      );
    }
    try {
      await next();
  ...
  private async createRuntimeActionRun(context: Context, actionId: string): Promise<Response> {
    ...
    const teamId = readOptionalTeamId(context, body);
    if (!policy.evaluate(action).allowed) {
      return writeRuntimeActionHttpResult(
        context,
        await this.executeRuntimeAction(actionId, input, connectionName, policy, runtimeGrant, teamId),
      );
    }
  ...
  private async executeRuntimeAction(
    ...
    try {
      const run = await this.options.actions.run({
        ...
        policy,
        runtimeTokenId: runtimeGrant?.tokenId,
        memberId: runtimeGrant?.memberId,
        teamId,
      });
      if (!run) {
  ...
  private async updateRuntimePolicy(context: Context): Promise<Response> {
    if (this.options.companyPolicyWriteOnly) {
      return jsonError(
        context,
        403,
        "policy_write_via_org_config",
        "Direct runtime-policy writes are disabled in company mode. Use PUT /api/org/config/policy.",
      );
    }
    const body = await readJsonBody(context, policyRequestMaxBytes);
...
function readOptionalTeamId(context: Context, body?: Record<string, unknown>): string | undefined {
  return (
    ...
    optionalString(context.req.header("x-omc-team-id")) ??
    optionalString(context.req.query("teamId"))
  );
}
...
function readRunLogListInput(context: Context): RunLogListQuery {
  ...
  const memberId = optionalString(context.req.query("memberId"));
  if (memberId !== undefined) {
    if (memberId.length > 128) {
      return { ok: false, message: "memberId must be at most 128 characters." };
    }
