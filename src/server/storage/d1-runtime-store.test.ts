...
describe("D1RuntimeDatabase", () => {
  ...
  it("filters runs by memberId from the JSON payload", async () => {
    const database = new D1RuntimeDatabase(new SqliteD1Database(), { runLimit: 5 });
    const match = {
      ...createRun("run-member", "2026-06-30T00:00:03.000Z"),
      memberId: "member-aaa",
    };
    await database.runLogStore.add(createRun("run-other", "2026-06-30T00:00:01.000Z"));
    await database.runLogStore.add(match);

    await expect(database.runLogStore.list({ memberId: "member-aaa" })).resolves.toMatchObject({
      items: [{ id: "run-member", memberId: "member-aaa" }],
    });
    await expect(database.runLogStore.list({ memberId: "missing" })).resolves.toMatchObject({
      items: [],
    });
  });
