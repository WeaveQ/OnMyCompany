...
import { cloudflareOmc, cloudflareOmcInt, resolvePublicOrigin } from "./cloudflare/cloudflare-env.ts";
...
export default {
  async fetch(request: Request, env: CloudflareEnv, _ctx: CloudflareExecutionContext): Promise<Response> {
    setPrivateNetworkAccessAllowed(parsePrivateNetworkAccessFlag(cloudflareOmc(env, "ALLOW_PRIVATE_NETWORK")));
...
async function createCloudflareApp(env: CloudflareEnv, publicOrigin: string): Promise<ConnectApp> {
  ...
  const secretCodec = await createSecretCodec(cloudflareOmc(env, "ENCRYPTION_KEY"));
  return await createConnectApp({
    ...
    runtimeDatabase: new D1RuntimeDatabase(env.DB, {
      secretCodec,
      runLimit: cloudflareOmcInt(env, "RUN_LIMIT", DEFAULT_RUN_LIMIT),
    }),
    transitFiles: (() => {
      const transitFileOptions = {
        publicOrigin,
        ttlSeconds: cloudflareOmcInt(env, "TRANSIT_FILE_TTL_SECONDS", 86_400),
        maxBytes: cloudflareOmcInt(env, "TRANSIT_FILE_MAX_BYTES", 100 * 1024 * 1024),
      };
    ...
    publicOrigin,
    secretCodec,
    adminToken: cloudflareOmc(env, "ADMIN_TOKEN"),
    runtimeToken: cloudflareOmc(env, "RUNTIME_TOKEN"),
    actionPolicy: new ActionPolicyService({
      allowedActions: parseActionPolicyList(cloudflareOmc(env, "ALLOWED_ACTIONS")),
      blockedActions: parseActionPolicyList(cloudflareOmc(env, "BLOCKED_ACTIONS")),
      allowedProxies: parseActionPolicyList(cloudflareOmc(env, "ALLOWED_PROXIES")),
      blockedProxies: parseActionPolicyList(cloudflareOmc(env, "BLOCKED_PROXIES")),
    }),
    logger: workerLogger,
...
function createCacheKey(env: CloudflareEnv, publicOrigin: string): string {
  return JSON.stringify({
    ...
    adminToken: cloudflareOmc(env, "ADMIN_TOKEN") ?? "",
    runtimeToken: cloudflareOmc(env, "RUNTIME_TOKEN") ?? "",
    encryptionKey: cloudflareOmc(env, "ENCRYPTION_KEY") ?? "",
    allowedActions: cloudflareOmc(env, "ALLOWED_ACTIONS") ?? "",
    blockedActions: cloudflareOmc(env, "BLOCKED_ACTIONS") ?? "",
    allowedProxies: cloudflareOmc(env, "ALLOWED_PROXIES") ?? "",
    blockedProxies: cloudflareOmc(env, "BLOCKED_PROXIES") ?? "",
    transitFileTtlSeconds: cloudflareOmc(env, "TRANSIT_FILE_TTL_SECONDS") ?? "",
    transitFileMaxBytes: cloudflareOmc(env, "TRANSIT_FILE_MAX_BYTES") ?? "",
    runLimit: cloudflareOmc(env, "RUN_LIMIT") ?? "",
  });
}
