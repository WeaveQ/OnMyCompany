...
export interface LoadCatalogOptions extends CreateCatalogStoreOptions {
  ...
  /**
   * When set, only these provider service ids are kept in the catalog store
   * (console list + action index). Used by OnMyCompany office profile.
   */
  allowedServices?: Iterable<string> | null;
}
...
export async function loadCatalog(
  ...
  if (options.allowedServices != null) {
    const allowed = new Set(options.allowedServices);
    providers = providers.filter((provider) => allowed.has(provider.service));
  }
