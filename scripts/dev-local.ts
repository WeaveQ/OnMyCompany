...
const webPort = process.env.OMC_WEB_PORT ?? "5180";
...
const processes: DevProcess[] = [
  ...
  startProcess(
    "web",
    npmCommand,
    ["run", "dev", "--workspace", "web", "--", "--clearScreen", "false", "--port", webPort, "--strictPort"],
    {},
  ),
];

console.log(`API runtime: http://127.0.0.1:${apiPort}`);
