...
/** Bundles provider icon mapping and caches it for the lifetime of the Vite process. */
...
async function loadProviderIconsModule(): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(`Could not load provider icons: ${response.status} ${response.statusText}`);
  }

  const payload = (await response.json()) as CatalogPayload;
  if (!Array.isArray(payload.items)) {
    throw new Error("Could not load provider icons: catalog.items is not an array");
  }
