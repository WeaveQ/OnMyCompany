...
export function SkillsPage(): ReactNode {
  ...
  async function setRoles(packageId: string): Promise<void> {
    const raw = window.prompt("可见角色（逗号分隔，留空=全部成员）", "admin,member");
    if (raw === null) return;
    const visibleToRoles = raw
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    setError(null);
            ...
            <div key={item.packageId} className="skills-row">
              ...
              {isAdmin ? (
                <div className="skills-row-actions">
                  ...
                  <Button variant="outline" size="sm" onClick={() => void setRoles(item.packageId)}>
                    角色
                  </Button>
