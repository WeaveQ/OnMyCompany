...
export function ActionsPage(props: ActionsPageProps): ReactNode {
  ...
  const serviceFromUrl = searchParams.get("service")?.trim() || null;
  ...
  function serviceQuery(extra?: { actionId?: string }): string {
  ...
  function selectService(value: string): void {
    const next = value === allProvidersFilterValue ? null : value;
  ...
  function renderActionRow(action: ActionDefinition): ReactNode {
    ...
    return (
      <Link
        ...
        to={serviceQuery({ actionId: action.id })}
      >
