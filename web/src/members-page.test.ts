...
/**
 * People IA (Phase 1):
 * Accounts = org account lifecycle
 * Team = current-team membership
 * Team directory = org team list
 * Source copy is English (locales hold zh/zh-TW).
 */
describe("people IA (accounts vs team vs directory)", () => {
  ...
  it("team page is membership-only (no org lifecycle APIs)", () => {
    ...
    expect(teamSrc).toMatch(/Add to team|Add member/);
  ...
  it("org teams directory is routed (not a separate sidebar item) and can create/open teams", () => {
    ...
    expect(orgTeamsSrc).toMatch(/Create team|创建团队/);
  ...
  it("team switcher supports company-wide sentinel", () => {
    expect(teamSrc).toContain("ALL_TEAMS_ID");
    expect(teamSrc).toContain("showAllTeams");
    expect(teamSrc).toContain("Company-wide");
  });
});
