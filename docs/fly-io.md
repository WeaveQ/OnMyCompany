...
OnMyCompany can run on Fly.io as the Node Docker runtime with persistent SQLite storage. Fly
...
```bash
OMC_ENCRYPTION_KEY=$(openssl rand -base64 32)
OMC_ADMIN_TOKEN=$(openssl rand -base64 32)
OMC_RUNTIME_TOKEN=$(openssl rand -base64 32)

fly secrets set \
  OMC_ORIGIN="https://my-open-connector.fly.dev" \
  OMC_ENCRYPTION_KEY="$OMC_ENCRYPTION_KEY" \
  OMC_ADMIN_TOKEN="$OMC_ADMIN_TOKEN" \
  OMC_RUNTIME_TOKEN="$OMC_RUNTIME_TOKEN" \
  --app my-open-connector
```

Keep `OMC_ENCRYPTION_KEY` in a password manager or another external secrets vault. If the
...
```bash
fly secrets set \
  OMC_ALLOWED_ACTIONS="github.*,hackernews.*" \
...
For OAuth2 providers, set `OMC_ORIGIN` to the public origin users will access. The runtime
builds provider callback URLs from that origin and `/oauth/callback`.

For example, with:

```bash
OMC_ORIGIN="https://api.example.com"
```
