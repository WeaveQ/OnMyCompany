...
configuring the OAuth app in Google Cloud. OnMyCompany only needs the resulting client id, client
secret, and a redirect URI that the OAuth app allows.

## Prerequisites

- The local Gateway runtime can run with Node.js 22 or newer.
...
If the runtime is reachable through a tunnel or another public origin, set `OMC_ORIGIN`
...
`OMC_ADMIN_TOKEN` or a runtime token, add the matching `Authorization: Bearer ...` header
...
Finish consent in the browser. After Gmail redirects back to the runtime, OnMyCompany stores the
...
Use the connector SDK client against a self-hosted OnMyCompany runtime. `baseUrl` is the server origin, not a
