...
这篇教程从你已经拥有 Gmail OAuth client 开始，不包含在 Google Cloud 创建或配置 OAuth app 的步骤。OnMyCompany 只需要这个 OAuth app 的 `clientId`、`clientSecret`，以及它允许当前 runtime 使用的 redirect URI。

## 前置条件

- 本地 Gateway runtime 可以在 Node.js 22 或更新版本上运行。
...
如果 runtime 通过 tunnel 或其他公网 origin 访问，启动前先设置 `OMC_ORIGIN`。redirect URI 会基于这个 origin 拼接 `/oauth/callback`。

```bash
OMC_ORIGIN="https://your-runtime.example" npm run dev
```
...
下面的示例默认使用 `http://localhost:3000`。如果你配置了 `OMC_ADMIN_TOKEN` 或 runtime token，对应的 admin 请求和 `/v1` 请求需要加上 `Authorization: Bearer ...` header。
...
在浏览器里完成 Gmail consent。Gmail 跳回 runtime 后，OnMyCompany 会把 OAuth credential 保存为默认 Gmail connection。
...
curl -s -X POST http://localhost:3000/v1/actions/gmail.search_threads \
  -H "authorization: Bearer $OMC_RUNTIME_TOKEN" \
...
自托管 OnMyCompany runtime 使用 connector SDK。`baseUrl` 是 server origin，不是 `/v1` URL。传入前面创建的 runtime token。
...
const open = new OpenConnector({
  ...
  runtimeToken: process.env.OMC_RUNTIME_TOKEN,
});
...
- `unauthorized`：在 Access 页面创建 runtime token，并通过 `OMC_RUNTIME_TOKEN` 传给 SDK。
