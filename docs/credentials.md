...
Set `OMC_DATA_DIR` to use another directory. The Docker image defaults this to
...
Set `OMC_ENCRYPTION_KEY` to encrypt stored credentials, OAuth client configuration, and
completed idempotent Action response payloads:

```bash
OMC_ENCRYPTION_KEY="replace-with-a-long-random-secret" npm run dev
```
...
Without `OMC_ENCRYPTION_KEY`, the runtime stays usable for local development and prints a
...
If the browser reaches the runtime through another origin, set `OMC_ORIGIN` before
starting the runtime:

```bash
OMC_ORIGIN="https://your-tunnel.example" npm run dev
```
...
```bash
OMC_ENCRYPTION_KEY="old-secret" \
OMC_NEW_ENCRYPTION_KEY="new-secret" \
...
when `OMC_ENCRYPTION_KEY` is configured.
...
```bash
OMC_ADMIN_TOKEN="replace-with-an-admin-token" npm run dev
```
...
`OMC_RUNTIME_TOKEN` is still accepted for bootstrap scripts and backward compatibility.
...
Use `OMC_ALLOWED_ACTIONS` to expose only selected actions to HTTP and MCP execution:

```bash
OMC_ALLOWED_ACTIONS="hackernews.*,github.get_current_user" npm run dev
```

Use `OMC_BLOCKED_ACTIONS` to deny specific actions even when a broader allowlist includes
them:

```bash
OMC_ALLOWED_ACTIONS="github.*" \
OMC_BLOCKED_ACTIONS="github.delete_repository" \
npm run dev
```
...
```bash
OMC_ALLOWED_PROXIES="github" npm run dev
```
...
Set `OMC_BLOCKED_PROXIES="*"` to disable `/v1/proxy/:service` entirely. Restrict both
surfaces when you want both restricted:

```bash
OMC_ALLOWED_ACTIONS="github.get_current_user" \
OMC_ALLOWED_PROXIES="github" \
npm run dev
```

Use `OMC_BLOCKED_PROXIES` to deny provider proxies even when `OMC_ALLOWED_PROXIES`
contains `*`:

```bash
OMC_ALLOWED_PROXIES="*" \
OMC_BLOCKED_PROXIES="github" \
npm run dev
```
