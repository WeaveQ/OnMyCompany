...
OnMyCompany 在 GitHub Packages 容器镜像仓库（GHCR）提供了预构建的 Docker 镜像，你无需克隆仓库或自己构建即可
运行 OpenConnector。镜像地址为：

```text
ghcr.io/oomol-lab/onmycompany
```
...
```bash
docker pull ghcr.io/oomol-lab/onmycompany:latest
```
...
先生成运行时 secret 并妥善保存。`OMC_ENCRYPTION_KEY` 用于加密存储的凭据、OAuth client 配置和已完成的
幂等 Action 响应；一旦丢失，`/app/data` 里加密的数据将无法恢复。`OMC_ADMIN_TOKEN` 用于 admin API 和
控制台的鉴权。

```bash
# 运行前请把两个值保存到密码管理器或 secrets vault。
export OMC_ENCRYPTION_KEY=$(openssl rand -base64 32)
export OMC_ADMIN_TOKEN=$(openssl rand -base64 32)
```
...
docker run -d \
  ...
  -e OMC_ORIGIN="https://api.example.com" \
  -e OMC_ENCRYPTION_KEY="$OMC_ENCRYPTION_KEY" \
  -e OMC_ADMIN_TOKEN="$OMC_ADMIN_TOKEN" \
  ghcr.io/oomol-lab/onmycompany:latest
```
