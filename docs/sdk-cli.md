...
For the self-hosted Gateway runtime, use `OpenConnector`. `baseUrl` is the server origin, not
a `/v1` URL:
...
Point connector-family commands at a self-hosted Gateway runtime:
