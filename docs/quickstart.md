...
If you expose the runtime through another origin, set `OMC_ORIGIN` before starting it.
...
For local development, open the Web Console at `http://localhost:5180`. The Vite dev server proxies
...
```bash
OMC_DATA_DIR=/path/to/data npm run dev
```
...
`allowedProxies` grant includes the provider service or `*`. `OMC_RUNTIME_TOKEN` remains
...
runtime rules use `OMC_ALLOWED_PROXIES` and `OMC_BLOCKED_PROXIES`; persistent
