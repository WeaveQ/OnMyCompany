...
The Node server accepts JWT access tokens when `OMC_JWKS_URI`,
`OMC_JWT_ISSUER`, and `OMC_JWT_AUDIENCE` are configured together. JWT
...
Admin endpoints under `/api/*`, `/docs`, and the Web Console use `OMC_ADMIN_TOKEN` when it
is configured.
...
under `OMC_DATA_DIR/files` and are cleaned up by age.
...
Deployment and runtime proxy access is controlled by `OMC_ALLOWED_PROXIES` and
`OMC_BLOCKED_PROXIES`; Action policy does not affect it. Persistent runtime tokens add an
...
An empty token grant denies proxy access, and `OMC_BLOCKED_PROXIES="*"` disables provider
