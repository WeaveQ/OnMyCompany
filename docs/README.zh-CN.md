...
OnMyCompany 是面向 AI Agent 的开源 connector gateway，也是 Pipedream/Composio 的开源替代方案。
...
<table>
  ...
  <tr>
    <td width="33.33%" align="center">🚀 <a href="https://example.invalid/docs/connector-saas/"><strong>托管</strong></a></td>
    <td width="33.33%" align="center"><a href="cloudflare.md"><strong>部署到 Cloudflare</strong></a></td>
    <td width="33.33%" align="center"><a href="https://example.invalid/docs/openconnector-self-hosting/"><strong>自托管</strong></a></td>
  </tr>
</table>
...
- 支持本地运行、Fly.io 部署、Cloudflare 兼容基础设施部署，也可以使用 托管 runtime。
...
- 部署方式覆盖本地 Docker 或 Node.js、Fly.io 持久化 SQLite、Cloudflare Workers/D1/R2/Static
  Assets，以及 托管 runtime。

## 适合什么场景

OnMyCompany 适合需要让 Agent 持续访问用户现有工具、但不想把 provider credential 交给 Agent 进程的产品。
...
OnMyCompany 内置本地 Dashboard，可用于浏览 connector、配置 credential、创建 runtime token 和查看运行时使用数据。

### Connector 预览

通过 connector catalog 可以查看可用服务、搜索 provider，并进入对应的 Action 与 credential 配置。

![OnMyCompany Connector 预览 Dashboard](../assets/open-console-zh.jpg)

### 数据统计概览

部署后，Overview 页面会集中展示运行时状态、可用 provider、可执行 Action、最近失败、工具调用趋势和最近调用记录。

![OnMyCompany 运行时概览 Dashboard](../assets/overview-page-zh.jpg)
...
[![一键部署 OnMyCompany 到 Cloudflare Workers](../assets/cloudflare-quickstart-video.png)](https://www.youtube.com/watch?v=R0V1ZdCuTgc)

[Cloudflare Workers 部署演示](https://www.youtube.com/watch?v=R0V1ZdCuTgc) 展示如何把
OnMyCompany 跑到 Cloudflare 的 Workers、D1、R2 和 Web 控制台上。视频流程与
...
> [OOMOL 官方托管 Connector](https://example.invalid/apps)。
...
这会拉取 `ghcr.io/oomol-lab/onmycompany:latest`。想改为从源码构建：
...
OnMyCompany 可以部署到 Cloudflare：Workers 运行 runtime，D1 保存状态，R2 处理中转文件，Static
Assets 承载 Web 控制台。
...
OnMyCompany 也可以部署到 Fly.io：使用 Node Docker runtime，并把 SQLite 数据持久化到 Fly volume。

Fly app 创建、volume、secret、部署、自定义域名和扩缩容步骤见 [fly-io.md](fly-io.md)。

## Docker 镜像（GHCR）

可以直接用 GitHub Packages（GHCR）上的预构建镜像运行 OnMyCompany：`ghcr.io/oomol-lab/onmycompany`。最新
...
OnMyCompany 与 [Wanta](https://github.com/oomol-lab/wanta) 是 OOMOL 开源生态中面向 AI Agent
的两个项目。OnMyCompany 负责把 Gmail、Slack、Notion 等外部服务接入 Agent；Wanta 则提供一套基于 OpenCode
运行的完整桌面 Agent 应用，并通过 OnMyCompany 使用已连接的 SaaS 服务。
...
`http://localhost:5180`，并把 API 请求代理到 runtime。
...
如果 OnMyCompany 对你有帮助，点个 ⭐ 可以帮助更多开发者发现这个项目。

<div align="center">

<img src="../assets/star-open-connector.gif" alt="在 GitHub 上为 OnMyCompany 点 Star 的操作演示" width="760" />

</div>

## 贡献者

感谢每一位参与建设 OnMyCompany 的贡献者。欢迎查看 [贡献指南](../CONTRIBUTING.md)，加入我们。

[![OnMyCompany 贡献者](https://contrib.rocks/image?repo=oomol-lab/onmycompany)](https://github.com/oomol-lab/onmycompany/graphs/contributors)
