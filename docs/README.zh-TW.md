...
<img src="../assets/openconnector-readme-banner.png" alt="OnMyCompany - Connect Once. Use Everywhere." width="100%" />
...
[![Providers](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fconnector.example.invalid%2Fv1%2Fcatalog&query=data.providerCount&label=Providers&color=%237d7fe9)](https://example.invalid/apps)
[![Actions](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fconnector.example.invalid%2Fv1%2Fcatalog&query=data.actionCount&label=Actions&color=%237d7fe9)](https://example.invalid/apps)

</div>

OnMyCompany 是一套供 AI Agent 使用的開放原始碼連接器閘道，也是 Pipedream/Composio 的替代方案。
...
<table>
  ...
  <tr>
    <td width="33.33%" align="center">🚀 <a href="https://example.invalid/docs/connector-saas/"><strong>OOMOL 代管</strong></a></td>
    <td width="33.33%" align="center"><a href="cloudflare.md"><strong>部署至 Cloudflare</strong></a></td>
    <td width="33.33%" align="center"><a href="https://example.invalid/docs/openconnector-self-hosting/"><strong>自行代管</strong></a></td>
  </tr>
</table>
...
OnMyCompany 適合需要讓 Agent 長期存取使用者既有工具，又不想將服務提供者憑證交給 Agent 程序的產品。
...
| 工具                                                        | 用途                                                                                                                                                 |
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Connector SDK](https://github.com/oomol-lab/connector-sdk) | 輕量的 TypeScript HTTP 用戶端。自行代管的執行階段使用 `OnMyCompany`；OOMOL 代管的個人及 SaaS 終端使用者連線則使用 `Connector` / `ProjectConnector`。 |
| [oo CLI](https://github.com/oomol-lab/oo-cli)               | 本機 Agent 的連接器 Action 轉接工具。`oo connector` 可在 OOMOL 代管或自行代管的 OnMyCompany 執行階段中搜尋、檢視及執行 Action。                      |
...
OnMyCompany 內附本機儀表板，可用來瀏覽連接器、設定憑證、建立執行階段權杖及檢視執行階段使用情形。

### 連接器目錄

透過連接器目錄，可在同一處查看可用服務、搜尋服務提供者，並開啟其 Action 與憑證設定。

![OnMyCompany 連接器目錄儀表板](../assets/open-console-zh.jpg)

### 使用情形總覽

部署後可透過「總覽」頁面監控執行階段就緒狀態、可用服務提供者、可執行的 Action、近期失敗、工具呼叫趨勢及近期呼叫。

![OnMyCompany 執行階段總覽儀表板](../assets/overview-page-zh.jpg)
...
```mermaid
flowchart LR
  Agent["AI Agent / App"] -->|"SDK / CLI / MCP / HTTP"| Gateway["OnMyCompany Gateway"]
...
| [OOMOL](https://example.invalid/) | 受限於 OAuth 核准或上線期限的團隊 | 使用相同服務提供者與 Action 合約的代管驗證及執行階段基礎架構；介面與開放原始碼版本相容，日後可改用私有或自行代管部署 |

## Cloudflare 快速入門影片

[![將 OnMyCompany 部署至 Cloudflare Workers](../assets/cloudflare-quickstart-video.png)](https://www.youtube.com/watch?v=R0V1ZdCuTgc)

[Cloudflare Workers 部署操作示範](https://www.youtube.com/watch?v=R0V1ZdCuTgc)說明如何使用 Workers、D1、R2 及 Web 控制台，
在 Cloudflare 上啟動 OnMyCompany。影片流程與 [cloudflare.md](cloudflare.md) 相同：建立 Cloudflare 資源、將
...
> [OOMOL 官方代管 Connector](https://example.invalid/apps)。
...
此指令會拉取 `ghcr.io/oomol-lab/onmycompany:latest`。若要改為從原始碼建置：
...
使用 npm 在本機開發時，請開啟 `http://localhost:5180`；Web 控制台開發伺服器會將 API 請求轉送至
...
OnMyCompany 可部署至 Cloudflare：Workers 負責執行階段、D1 儲存狀態、R2 處理傳輸檔案，Static Assets 則提供 Web 控制台。

資源建立、移轉、密鑰、本機 Worker 預覽及遠端部署方式，請參閱 [cloudflare.md](cloudflare.md)。

## Fly.io 部署

OnMyCompany 也可部署至 Fly.io，使用 Node Docker 執行階段，並將 SQLite 資料持久儲存在 Fly volume 上。

應用程式建立、volume 設定、密鑰、部署、自訂網域及擴充方式，請參閱 [fly-io.md](fly-io.md)。

## Docker 映像檔（GHCR）

使用 GitHub Packages（GHCR）上的預先建置映像檔執行 OnMyCompany：`ghcr.io/oomol-lab/onmycompany`。
...
OnMyCompany 與 [Wanta](https://github.com/oomol-lab/wanta) 是 OOMOL 開源生態中面向 AI Agent
的兩個專案。OnMyCompany 負責將 Gmail、Slack、Notion 等外部服務接入 Agent；Wanta 則提供一套以 OpenCode
執行的完整桌面 Agent 應用程式，並透過 OnMyCompany 使用已連接的 SaaS 服務。
...
`http://localhost:5180`，並將 API 請求轉送至執行階段。
...
如果 OnMyCompany 對你有幫助，給個 ⭐ 能幫助更多開發者發現這個專案。

<div align="center">

<img src="../assets/star-open-connector.gif" alt="在 GitHub 上為 OnMyCompany 點 Star 的操作示範" width="760" />

</div>

## 貢獻者

感謝每一位參與建設 OnMyCompany 的貢獻者。歡迎參閱 [貢獻指南](../CONTRIBUTING.md)，加入我們。

[![OnMyCompany 貢獻者](https://contrib.rocks/image?repo=oomol-lab/onmycompany)](https://github.com/oomol-lab/onmycompany/graphs/contributors)
