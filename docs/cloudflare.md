...
```dotenv
OMC_ADMIN_TOKEN=replace-with-a-local-admin-token
OMC_ENCRYPTION_KEY=replace-with-a-local-encryption-key
```
...
```bash
curl https://onmycompany.example.workers.dev/health
```
...
deletes it automatically. KV clamps `OMC_TRANSIT_FILE_TTL_SECONDS` to a minimum of 60
seconds and `OMC_TRANSIT_FILE_MAX_BYTES` to a maximum of 25 MiB.
...
file limits, and credential encryption. `PORT`, `HOST`, and `OMC_DATA_DIR` are local
